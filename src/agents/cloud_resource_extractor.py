...
class CloudResourceExtractor:
    """
    Agent for extracting cloud resource data based on parameters.
    """
    def __init__(self, data_dir="src/data/input", output_data_dir="src/data/output"):
        """
        Initialize the cloud resource data extractor.
        ...
        self.agent_name = "Cloud Resource Extractor"
        self.extraction_params = None
        self.cloud_data = None
        self.filtered_cloud_data = None
        self.cloud_resource_inventory = None  # To store the cloud resource inventory
    ...
    def run(self):
        ...
        try:
            ...
            # Step 4: Filter cloud resource data based on system names
            if not self._filter_cloud_data():
                return False
    ...
    def _load_data(self):
        ...
        try:
            ...
            if not os.path.exists(self.data_file_path):
                print(f"Cloud resource data file not found: {self.data_file_path}")
                return False
            ...
            # Read cloud resource data from Cloud_Resource_Inventory sheet
            self.cloud_data = pd.read_excel(self.data_file_path, sheet_name="Cloud_Resource_Inventory")
            print(f"Loaded cloud resource data with {len(self.cloud_data)} records")
            
            # Also load the cloud resource inventory (same as cloud_data in this case)
            self.cloud_resource_inventory = self.cloud_data.copy()
    ...
    def _validate_data(self):
        ...
        """
        if self.extraction_params is None or self.cloud_data is None:
        ...
        if self.cloud_data.empty:
            print("Cloud resource data dataframe is empty")
            return False
        ...
        # Check for required columns in cloud_data
        ...
        if missing_columns:
            print(f"Missing required columns in cloud resource data: {', '.join(missing_columns)}")
            return False
    ...
    def _filter_cloud_data(self):
        """
        Filter cloud resource data based on system names from extraction parameters.
        ...
        for _, row in self.extraction_params.iterrows():
            if 'System Name' in row and pd.notna(row['System Name']):
                if row['System Name'].strip().lower() == 'all':
                    ...
                    self.filtered_cloud_data = self.cloud_data
                    ...
                    self.filtered_cloud_data = self.cloud_data[self.cloud_data['System Name'].isin(system_names)]
        
        if self.filtered_cloud_data is None or self.filtered_cloud_data.empty:
    ...
    def _prepare_output_path(self):
        ...
        """
        self.output_file = os.path.join(self.output_data_dir, f"{self.client_name}_Cloud_Resource_Data.xlsx")
    ...
    def save_verified_population_file(self):
        ...
        """
        if self.filtered_cloud_data is None or self.output_file is None:
        ...
        try:
            ...
            with pd.ExcelWriter(self.output_file) as writer:
                # Write the extracted cloud resource data to the "Cloud_Resource_Inventory" sheet
                self.filtered_cloud_data.to_excel(writer, sheet_name="Cloud_Resource_Inventory", index=False)
    ...
    def _create_metadata(self):
        ...
        hash_total = self._calculate_hash_total(self.filtered_cloud_data)
        ...
        metadata_entry = {
            ...
            'Record count': [len(self.filtered_cloud_data)],
