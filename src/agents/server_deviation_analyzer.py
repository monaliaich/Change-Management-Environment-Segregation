...
class ServerDeviationAnalyzer:
    """
    Agent for analyzing server deviations in the extracted data.
    This agent checks if each system has proper server instance segregation.
    """
    def __init__(self, input_dir="src/data/output", output_dir="src/data/output"):
        """
        Initialize the server deviation analyzer.
        
        Args:
            input_dir: Directory containing input files (output from extraction process)
            output_dir: Directory for output files
        """
        self.name = "ServerDeviationAnalyzer"
        self.logger = logging.getLogger(self.name)
        self.input_dir = input_dir
        self.output_dir = output_dir
        self.input_file = None
        self.server_data = None
        self.server_instance_mapping = None
    ...
    def find_latest_input_file(self):
        """
        Find the latest Server_Data file in the input directory.
        
        Returns:
            bool: True if a file was found, False otherwise
        """
        try:
            # Look for files matching the pattern *Server_Data.xlsx
            pattern = os.path.join(self.input_dir, "*Server_Data.xlsx")
            matching_files = glob.glob(pattern)
            
            if not matching_files:
                self.logger.error(f"No Server_Data files found in {self.input_dir}")
                return False
            ...
            self.client_name = filename.split('_Server_Data')[0]
    ...
    def load_data(self):
        ...
        try:
            ...
            # Check if Server_Instance_Mapping sheet exists
            if "Server_Instance_Mapping" not in excel_file.sheet_names:
                self.logger.error("Required sheet 'Server_Instance_Mapping' not found in input file")
                return False
            
            # Load the Server_Instance_Mapping sheet
            self.server_data = pd.read_excel(excel_file, sheet_name="Server_Instance_Mapping")
            
            # Check if required columns exist
            required_columns = ["System Name", "Environment Type", "Env-ID", "Server/Instance ID", "Hostname"]
            missing_columns = [col for col in required_columns if col not in self.server_data.columns]
            if missing_columns:
                self.logger.error(f"Required columns {', '.join(missing_columns)} not found in Server_Instance_Mapping sheet")
                return False
            
            self.logger.info(f"Loaded {len(self.server_data)} records from {self.input_file}")
            return True
        except Exception as e:
            self.logger.error(f"Error loading data: {str(e)}")
            return False 
        
    def analyze_server_deviations(self):
        """
        Analyze server deviations using Azure AI Foundry.
        
        Returns:
            bool: True if analysis was successful, False otherwise
        """
        try:
            if self.server_data is None or self.server_data.empty:
            ...
            self.logger.error(f"Error analyzing server deviations: {str(e)}")
            self.logger.error(f"Traceback: {traceback.format_exc()}")
            return False   

    def _prepare_data_for_analysis(self):
        """
        Prepare data for server deviation analysis.
        
        Returns:
            DataFrame: Prepared data for analysis
        """
        # Group by System Name to get unique systems
        systems = self.server_data['System Name'].unique()
        
        # Create a list of dictionaries with system name and its environments
        data_for_analysis = []
        for system in systems:
            system_data = self.server_data[self.server_data['System Name'] == system]
            
            # Get list of environments for this system
            system_envs = system_data['Environment Type'].unique().tolist()
            
            # Create a clear representation for analysis
            data_for_analysis.append({
                'System Name': system,
                'Environment Types': system_envs,
                # Include explicit flags for clarity
                'Has DEV Environment': 'DEV' in system_envs,
                'Has TEST Environment': 'TEST' in system_envs,
                'Has PROD Environment': 'PROD' in system_envs
            })
    ...
    def _process_batches_with_ai(self, data):
        ...
        for i in range(0, len(data), batch_size):
            ...
            prompt = self._create_server_analysis_prompt(batch_json)
    ...
    async def _call_ai_for_analysis_async(self, prompt, batch_index, batch_size, total_records):
        ...
        try:
            ...
            system_prompt = """You are an expert IT server segregation analyst specializing in compliance and server validation. 
            Your task is to analyze system server environments and identify deviations from the required DEV, TEST, PROD environment setup."""
    ...
    def _create_server_analysis_prompt(self, data_json):
        """
        Create the prompt for server analysis.
        
        Args:
            data_json: JSON string containing server data
            
        Returns:
            String prompt for the AI model
        """
        return f"""
        You are an expert in IT server environment segregation analysis. I need you to analyze the following system server environment data:
        
        {data_json}
        
        TASK:
        For each System Name, check if it has all three required environments: DEV, TEST, and PROD.
        
        INSTRUCTIONS:
        1. For each system, look at the boolean fields 'Has DEV Environment', 'Has TEST Environment', and 'Has PROD Environment'.
        2. If all three are True, the system has all required environments.
        3. If any are False, note which environments are missing.
        
        REQUIRED OUTPUT FORMAT:
        A JSON array containing one object for each System Name, with these exact fields:
        - System_Name: The system name
        - Server_Config: Either "Deviation" or "OK"
        - Reason: The reason for deviation, or "DEV, TEST, PROD environments are present" if OK
        
        EXAMPLES:
        1. If a system has all environments:
        {{"System_Name": "SAP FI", "Server_Config": "OK", "Reason": "DEV, TEST, PROD environments are present"}}
        
        2. If a system is missing DEV:
        {{"System_Name": "Oracle EBS AP", "Server_Config": "Deviation", "Reason": "No DEV environment available"}}
        
        3. If a system is missing multiple environments:
        {{"System_Name": "Custom App", "Server_Config": "Deviation", "Reason": "No DEV and TEST environments available"}}
        
        IMPORTANT:
        - Focus ONLY on the presence of required environments
        - DO NOT analyze any other aspects like shared servers or hostnames
    ...
    def _call_ai_with_thread_and_run(self, system_prompt, user_prompt, max_retries=3):
        ...
        while retry_count < max_retries:
            try:
                ...
                system_instructions = f"You are an expert in IT server environment segregation analysis. {system_prompt}"
        ...
        try:
            ...
            if not agents_list:
                ...
                agent = self.client.create_agent(
                    name="Server Deviation Analyzer",
                    description="Analyzes server environment deviations in IT systems",
                    model=self.AGENT_MODEL_DEPLOYMENT_NAME
                )
                return agent.id
            else:
    ...
    def save_analysis_results(self):
        ...
        try:
            ...
            if not self.input_file or not self.server_data.any().any():
            ...
            with pd.ExcelWriter(output_file_path, engine='openpyxl') as writer:
                # Write the input server data to the first sheet
                self.server_data.to_excel(writer, sheet_name="Server_Instance_Mapping", index=False)
                
                # Write the analysis results to the second sheet
                self.analysis_results.to_excel(writer, sheet_name="Server_Deviation_Analysis", index=False)
    ...
    def _create_metadata(self):
        ...
        if self.analysis_results is not None and not self.analysis_results.empty:
            ...
            exception_records = len(self.analysis_results[self.analysis_results['Server_Config'] == 'Deviation'])
            ok_records = len(self.analysis_results[self.analysis_results['Server_Config'] == 'OK'])
    ...
    def run(self):
        """
        Run the complete server deviation analysis workflow.
        ...
        try:
            ...
            # Step 2: Analyze server deviations
            success = self.analyze_server_deviations()
            if not success:
                self.logger.error("Failed to analyze server deviations")
                return False
            ...
            self.logger.info(f"Server Deviation Analysis completed successfully. Results saved to {output_file}")
            return True
