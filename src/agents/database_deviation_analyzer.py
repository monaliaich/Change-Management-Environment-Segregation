...
class DatabaseDeviationAnalyzer:
    """
    Agent for analyzing database deviations in the extracted data.
    This agent checks if each system has proper database instance segregation.
    """
    def __init__(self, input_dir="src/data/output", output_dir="src/data/output"):
        """
        Initialize the database deviation analyzer.
        
        Args:
            input_dir: Directory containing input files (output from extraction process)
            output_dir: Directory for output files
        """
        self.name = "DatabaseDeviationAnalyzer"
        self.logger = logging.getLogger(self.name)
        self.input_dir = input_dir
        self.output_dir = output_dir
        self.input_file = None
        self.db_data = None
        self.db_config_mapping = None
    ...
    def find_latest_input_file(self):
        """
        Find the latest Database_Data file in the input directory.
        
        Returns:
            bool: True if a file was found, False otherwise
        """
        try:
            # Look for files matching the pattern *Database_Data.xlsx
            pattern = os.path.join(self.input_dir, "*Database_Data.xlsx")
            matching_files = glob.glob(pattern)
            
            if not matching_files:
                self.logger.error(f"No Database_Data files found in {self.input_dir}")
                return False
            ...
            self.client_name = filename.split('_Database_Data')[0]
    ...
    def load_data(self):
        ...
        try:
            ...
            if "Database_Config_Mapping" not in excel_file.sheet_names:
                self.logger.error("Required sheet 'Database_Config_Mapping' not found in input file")
                return False
            
            # Load the Database_Config_Mapping sheet
            ...
            self.logger.info(f"Loaded {len(self.db_data)} records from {self.input_file}")
            return True
        except Exception as e:
    ...
    def analyze_database_deviations(self):
        """
        Analyze database deviations using Azure AI Foundry.
        ...
        try:
            ...
            self.logger.error(f"Error analyzing database deviations: {str(e)}")
            self.logger.error(f"Traceback: {traceback.format_exc()}")
            return False   

    def _prepare_data_for_analysis(self):
        """
        Prepare data for database deviation analysis.
        
        Returns:
            DataFrame: Prepared data for analysis
        """
        # Group by System Name to get unique systems
        systems = self.db_data['System Name'].unique()
        
        # Create a list of dictionaries with system name and its environments
        data_for_analysis = []
        for system in systems:
            system_data = self.db_data[self.db_data['System Name'] == system]
            
            # Get list of environments for this system
            system_envs = system_data['Environment Type'].tolist()
            
            # Get list of Env-IDs for this system
            system_env_ids = system_data['Env-ID'].tolist()
            
            data_for_analysis.append({
                'System Name': system,
                'Environments': system_envs,
                'Env-IDs': system_env_ids
            })
    ...
    def _process_batches_with_ai(self, data):
        ...
        for i in range(0, len(data), batch_size):
            ...
            prompt = self._create_database_analysis_prompt(batch_json)
    ...
    async def _call_ai_for_analysis_async(self, prompt, batch_index, batch_size, total_records):
        ...
        try:
            ...
            system_prompt = """You are an expert IT database segregation analyst specializing in compliance and database validation. 
            Your task is to analyze system database environments and identify deviations from the required DEV, TEST, PROD environment setup."""
    ...
    def _create_database_analysis_prompt(self, data_json):
        """
        Create the prompt for database analysis.
        
        Args:
            data_json: JSON string containing database data
            
        Returns:
            String prompt for the AI model
        """
        return f"""
        You are an expert in IT database environment segregation analysis. I need you to analyze the following system database environment data:
        ...
        - Database_Config: Either "Deviation" or "OK"
        - Reason: The reason for deviation, or "DEV, TEST, PROD environments are present" if OK
        
        EXAMPLE OUTPUT:
        [
            {{"System_Name": "SAP FI", "Database_Config": "OK", "Reason": "DEV, TEST, PROD environments are present"}},
            {{"System_Name": "Oracle EBS AP", "Database_Config": "Deviation", "Reason": "No TEST environment available"}}
        ]
    ...
    def _call_ai_with_thread_and_run(self, system_prompt, user_prompt, max_retries=3):
        ...
        while retry_count < max_retries:
            try:
                ...
                system_instructions = f"You are an expert in IT database environment segregation analysis. {system_prompt}"
        ...
        try:
            ...
            if not agents_list:
                ...
                agent = self.client.create_agent(
                    name="Database Deviation Analyzer",
                    description="Analyzes database environment deviations in IT systems",
                    model=self.AGENT_MODEL_DEPLOYMENT_NAME
                )
                return agent.id
            else:
    ...
    def save_analysis_results(self):
        ...
        try:
            ...
            if not self.input_file or not self.db_data.any().any():
            ...
            with pd.ExcelWriter(output_file_path, engine='openpyxl') as writer:
                # Write the input database data to the first sheet
                self.db_data.to_excel(writer, sheet_name="Database_Config_Mapping", index=False)
                
                # Write the analysis results to the second sheet
                self.analysis_results.to_excel(writer, sheet_name="Database_Deviation_Analysis", index=False)
    ...
    def _create_metadata(self):
        ...
        if self.analysis_results is not None and not self.analysis_results.empty:
            ...
            exception_records = len(self.analysis_results[self.analysis_results['Database_Config'] == 'Deviation'])
            ok_records = len(self.analysis_results[self.analysis_results['Database_Config'] == 'OK'])
    ...
    def run(self):
        """
        Run the complete database deviation analysis workflow.
        
        Returns:
            bool: True if analysis was successful, False otherwise
        """
        try:
            self.logger.info("Starting Database Deviation Analysis")
            ...
            # Step 2: Analyze database deviations
            success = self.analyze_database_deviations()
            if not success:
                self.logger.error("Failed to analyze database deviations")
                return False
            ...
            self.logger.info(f"Database Deviation Analysis completed successfully. Results saved to {output_file}")
            return True
            
        except Exception as e:
            self.logger.error(f"Error running Database Deviation Analysis: {str(e)}")
            self.logger.error(f"Traceback: {traceback.format_exc()}")
            return False