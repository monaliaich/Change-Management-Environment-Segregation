...
class EnvironmentDataExtractor:
    """
    Agent for extracting environment data based on parameters.
    """
    def __init__(self, data_dir="src/data/input", output_data_dir="src/data/output"):
        """
        Initialize the environment data extractor.
        ...
        self.agent_name = "Environment Data Extractor"
        self.extraction_params = None
        self.env_data = None
        self.filtered_env_data = None
    ...
    def run(self):
        ...
        try:
            ...
            # Step 4: Filter environment register data based on system names
            if not self._filter_environment_data():
                return False
    ...
    def _load_data(self):
        ...
        try:
            ...
            if not os.path.exists(self.data_file_path):
                print(f"Environment data file not found: {self.data_file_path}")
                return False
            
            # Read extraction parameters from Sheet1
            self.extraction_params = pd.read_excel(self.extraction_file_path, sheet_name="Sheet1")
            
            # Read environment data from Environment_Register
            self.env_data = pd.read_excel(self.data_file_path, sheet_name="Environment_Register")
            
            return True
            
        except PermissionError as e:
            print(f"Permission denied: Cannot access file. Make sure the file is not open in another program: {str(e)}")
            return False
    ...
    def _validate_data(self):
        ...
        """
        if self.extraction_params is None or self.env_data is None:
            print("No data to validate")
            return False
            
        validation_result, validation_message = validate_data_content(self.extraction_params, self.env_data)
    ...
    def _filter_environment_data(self):
        """
        Filter environment register data based on system names from extraction parameters.
        ...
        for _, row in self.extraction_params.iterrows():
            if 'System Name' in row and pd.notna(row['System Name']):
                if row['System Name'].strip().lower() == 'all':
                    ...
                    self.filtered_env_data = self.env_data
                    ...
                    self.filtered_env_data = self.env_data[self.env_data['System Name'].isin(system_names)]
        
        if self.filtered_env_data is None or self.filtered_env_data.empty:
    ...
    def _prepare_output_path(self):
        ...
        """
        self.output_file = os.path.join(self.output_data_dir, f"{self.client_name}_Environment_Data.xlsx")
    ...
    def save_verified_population_file(self):
        ...
        """
        if self.filtered_env_data is None or self.output_file is None:
        ...
        try:
            ...
            with pd.ExcelWriter(self.output_file) as writer:
                # Write the extracted environment register data to the "Environment_Register" sheet
                self.filtered_env_data.to_excel(writer, sheet_name="Environment_Register", index=False)
    ...
    def _create_metadata(self):
        ...
        hash_total = self._calculate_hash_total(self.filtered_env_data)
        ...
        metadata_entry = {
            ...
            'Record count': [len(self.filtered_env_data)],
