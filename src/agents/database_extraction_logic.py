...
class DatabaseDataExtractor:
    """
    Agent for extracting database data based on parameters.
    """
    def __init__(self, data_dir="src/data/input", output_data_dir="src/data/output"):
        """
        Initialize the database data extractor.
        ...
        self.agent_name = "Database Data Extractor"
        self.extraction_params = None
        self.db_data = None
        self.filtered_db_data = None
        self.db_config_mapping = None  # Added to store the database config mapping
    ...
    def run(self):
        ...
        try:
            ...
            # Step 4: Filter database data based on system names
            if not self._filter_database_data():
                return False
    ...
    def _load_data(self):
        ...
        try:
            ...
            if not os.path.exists(self.data_file_path):
                print(f"Database data file not found: {self.data_file_path}")
                return False
            ...
            # Read database data from Database_Config_Mapping sheet
            self.db_data = pd.read_excel(self.data_file_path, sheet_name="Database_Config_Mapping")
            print(f"Loaded database data with {len(self.db_data)} records")
            
            # Also load the database config mapping (same as db_data in this case)
            self.db_config_mapping = self.db_data.copy()
    ...
    def _validate_data(self):
        ...
        """
        if self.extraction_params is None or self.db_data is None:
        ...
        if self.db_data.empty:
            print("Database data dataframe is empty")
            return False
        ...
        # Check for required columns in db_data
        ...
        if missing_columns:
            print(f"Missing required columns in database data: {', '.join(missing_columns)}")
            return False
    ...
    def _filter_database_data(self):
        """
        Filter database data based on system names from extraction parameters.
        ...
        for _, row in self.extraction_params.iterrows():
            if 'System Name' in row and pd.notna(row['System Name']):
                if row['System Name'].strip().lower() == 'all':
                    ...
                    self.filtered_db_data = self.db_data
                    ...
                    self.filtered_db_data = self.db_data[self.db_data['System Name'].isin(system_names)]
        
        if self.filtered_db_data is None or self.filtered_db_data.empty:
    ...
    def _prepare_output_path(self):
        ...
        """
        self.output_file = os.path.join(self.output_data_dir, f"{self.client_name}_Database_Data.xlsx")
    ...
    def save_verified_population_file(self):
        ...
        """
        if self.filtered_db_data is None or self.output_file is None:
        ...
        try:
            ...
            with pd.ExcelWriter(self.output_file) as writer:
                # Write the extracted database data to the "Database_Config_Mapping" sheet
                self.filtered_db_data.to_excel(writer, sheet_name="Database_Config_Mapping", index=False)
    ...
    def _create_metadata(self):
        ...
        hash_total = self._calculate_hash_total(self.filtered_db_data)
        ...
        metadata_entry = {
            ...
            'Record count': [len(self.filtered_db_data)],
