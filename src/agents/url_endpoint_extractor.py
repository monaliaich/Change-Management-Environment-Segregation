...
class URLEndpointExtractor:
    """
    Agent for extracting URL endpoint data based on parameters.
    """
    def __init__(self, data_dir="src/data/input", output_data_dir="src/data/output"):
        """
        Initialize the URL endpoint data extractor.
        ...
        self.agent_name = "URL Endpoint Extractor"
        self.extraction_params = None
        self.url_data = None
        self.filtered_url_data = None
        self.url_endpoint_listing = None  # To store the URL endpoint listing
    ...
    def run(self):
        ...
        try:
            ...
            # Step 4: Filter URL endpoint data based on system names
            if not self._filter_url_data():
                return False
    ...
    def _load_data(self):
        ...
        try:
            ...
            if not os.path.exists(self.data_file_path):
                print(f"URL endpoint data file not found: {self.data_file_path}")
                return False
            ...
            # Read URL endpoint data from URL_Endpoint_Listing sheet
            self.url_data = pd.read_excel(self.data_file_path, sheet_name="URL_Endpoint_Listing")
            print(f"Loaded URL endpoint data with {len(self.url_data)} records")
            
            # Also load the URL endpoint listing (same as url_data in this case)
            self.url_endpoint_listing = self.url_data.copy()
    ...
    def _validate_data(self):
        ...
        """
        if self.extraction_params is None or self.url_data is None:
        ...
        if self.url_data.empty:
            print("URL endpoint data dataframe is empty")
            return False
        ...
        # Check for required columns in url_data
        ...
        if missing_columns:
            print(f"Missing required columns in URL endpoint data: {', '.join(missing_columns)}")
            return False
    ...
    def _filter_url_data(self):
        """
        Filter URL endpoint data based on system names from extraction parameters.
        ...
        for _, row in self.extraction_params.iterrows():
            if 'System Name' in row and pd.notna(row['System Name']):
                if row['System Name'].strip().lower() == 'all':
                    ...
                    self.filtered_url_data = self.url_data
                    ...
                    self.filtered_url_data = self.url_data[self.url_data['System Name'].isin(system_names)]
        
        if self.filtered_url_data is None or self.filtered_url_data.empty:
    ...
    def _prepare_output_path(self):
        ...
        """
        self.output_file = os.path.join(self.output_data_dir, f"{self.client_name}_URL_Endpoint_Data.xlsx")
    ...
    def save_verified_population_file(self):
        ...
        """
        if self.filtered_url_data is None or self.output_file is None:
        ...
        try:
            ...
            with pd.ExcelWriter(self.output_file) as writer:
                # Write the extracted URL endpoint data to the "URL_Endpoint_Listing" sheet
                self.filtered_url_data.to_excel(writer, sheet_name="URL_Endpoint_Listing", index=False)
    ...
    def _create_metadata(self):
        ...
        hash_total = self._calculate_hash_total(self.filtered_url_data)
        ...
        metadata_entry = {
            ...
            'Record count': [len(self.filtered_url_data)],
