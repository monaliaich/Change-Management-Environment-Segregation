...
class URLEndpointDeviationAnalyzer:
    """
    Agent for analyzing URL endpoint deviations in the extracted data.
    This agent checks if each system has proper URL endpoint segregation.
    """
    def __init__(self, input_dir="src/data/output", output_dir="src/data/output"):
        """
        Initialize the URL endpoint deviation analyzer.
        
        Args:
            input_dir: Directory containing input files (output from extraction process)
            output_dir: Directory for output files
        """
        self.name = "URLEndpointDeviationAnalyzer"
        self.logger = logging.getLogger(self.name)
        self.input_dir = input_dir
        self.output_dir = output_dir
        self.input_file = None
        self.url_data = None
        self.url_endpoint_listing = None
    ...
    def find_latest_input_file(self):
        """
        Find the latest URL_Endpoint_Data file in the input directory.
        
        Returns:
            bool: True if a file was found, False otherwise
        """
        try:
            # Look for files matching the pattern *URL_Endpoint_Data.xlsx
            pattern = os.path.join(self.input_dir, "*URL_Endpoint_Data.xlsx")
            matching_files = glob.glob(pattern)
            
            if not matching_files:
                self.logger.error(f"No URL_Endpoint_Data files found in {self.input_dir}")
                return False
            ...
            self.client_name = filename.split('_URL_Endpoint_Data')[0]
    ...
    def load_data(self):
        ...
        try:
            ...
            # Load the URL_Endpoint_Listing sheet
            self.url_data = pd.read_excel(excel_file, sheet_name="URL_Endpoint_Listing")
            
            # Check if required columns exist
            required_columns = ["System Name", "Environment Type", "URL/Endpoint"]
            missing_columns = [col for col in required_columns if col not in self.url_data.columns]
            if missing_columns:
                self.logger.error(f"Required columns {', '.join(missing_columns)} not found in URL_Endpoint_Listing sheet")
                return False
            
            self.logger.info(f"Loaded {len(self.url_data)} records from {self.input_file}")
            return True
        except Exception as e:
            self.logger.error(f"Error loading data: {str(e)}")
            return False 

    def analyze_url_deviations(self):
        """
        Analyze URL endpoint deviations using Azure AI Foundry.
        
        Returns:
            bool: True if analysis was successful, False otherwise
        """
        try:
            if self.url_data is None or self.url_data.empty:
            ...
            self.logger.error(f"Error analyzing URL endpoint deviations: {str(e)}")
            self.logger.error(f"Traceback: {traceback.format_exc()}")
            return False   

    def _prepare_data_for_analysis(self):
        """
        Prepare data for URL endpoint deviation analysis, focusing only on environment types.
        
        Returns:
            DataFrame: Prepared data for analysis
        """
        # Group by System Name to get unique systems
        systems = self.url_data['System Name'].unique()
        
        # Create a list of dictionaries with system name and its environments
        data_for_analysis = []
        for system in systems:
            system_data = self.url_data[self.url_data['System Name'] == system]
    ...
    def _process_batches_with_ai(self, data):
        ...
        for i in range(0, len(data), batch_size):
            ...
            prompt = self._create_url_analysis_prompt(batch_json)
    ...
    async def _call_ai_for_analysis_async(self, prompt, batch_index, batch_size, total_records):
        ...
        try:
            ...
            system_prompt = """You are an expert IT URL endpoint segregation analyst specializing in compliance and URL validation. 
            Your task is to analyze system URL endpoints and identify deviations from the required environment setup."""
    ...
    def _create_url_analysis_prompt(self, data_json):
        """
        Create the prompt for environment type analysis.
        
        Args:
            data_json: JSON string containing environment type data
            
        Returns:
            String prompt for the AI model
        """
        return f"""
        You are analyzing IT system environments. I need you to check if each system has all required environment types.
        ...
        TASK:
        For each System Name, check ONLY if it has all three required environment types: DEV, TEST, and PROD.
        ...
        - URL_Config: Either "Deviation" or "OK"
        ...
        {{"System_Name": "Workday Payroll", "URL_Config": "OK", "Reason": "DEV, TEST, PROD environments are present"}}
        
        2. If a system is missing TEST (Has DEV=True, Has TEST=False, Has PROD=True):
        {{"System_Name": "SAP FI", "URL_Config": "Deviation", "Reason": "No TEST environment available"}}
        
        3. If a system is missing multiple environments (Has DEV=False, Has TEST=False, Has PROD=True):
        {{"System_Name": "Oracle EBS AP", "URL_Config": "Deviation", "Reason": "No DEV and TEST environments available"}}
        
        CRITICAL:
        - Focus ONLY on the presence of environment types (DEV, TEST, PROD)
        - DO NOT mention URLs or endpoints in your analysis
    ...
    def _call_ai_with_thread_and_run(self, system_prompt, user_prompt, max_retries=3):
        ...
        while retry_count < max_retries:
            try:
                ...
                system_instructions = f"You are an expert in IT URL endpoint segregation analysis. {system_prompt}"
        ...
        try:
            ...
            if not agents_list:
                ...
                agent = self.client.create_agent(
                    name="URL Endpoint Deviation Analyzer",
                    description="Analyzes URL endpoint environment deviations in IT systems",
                    model=self.AGENT_MODEL_DEPLOYMENT_NAME
                )
                return agent.id
            else:
    ...
    def save_analysis_results(self):
        ...
        try:
            ...
            if not self.input_file or not self.url_data.any().any():
            ...
            with pd.ExcelWriter(output_file_path, engine='openpyxl') as writer:
                ...
                self.analysis_results.to_excel(writer, sheet_name="URL_Endpoint_Deviation_Analysis", index=False)
    ...
    def _create_metadata(self):
        ...
        if self.analysis_results is not None and not self.analysis_results.empty:
            ...
            exception_records = len(self.analysis_results[self.analysis_results['URL_Config'] == 'Deviation'])
            ok_records = len(self.analysis_results[self.analysis_results['URL_Config'] == 'OK'])
    ...
    def run(self):
        """
        Run the complete URL endpoint deviation analysis workflow.
        ...
        try:
            ...
            # Step 2: Analyze URL endpoint deviations
            success = self.analyze_url_deviations()
            if not success:
                self.logger.error("Failed to analyze URL endpoint deviations")
                return False
            ...
            self.logger.info(f"URL Endpoint Deviation Analysis completed successfully. Results saved to {output_file}")
            return True
