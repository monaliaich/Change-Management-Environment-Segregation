...
class EnvironmentDeviationAnalyzer:
    """
    Agent for analyzing environment deviations in the extracted data.
    This agent checks if each system has all required environments (DEV, TEST, PROD).
    """
    def __init__(self, input_dir="src/data/output", output_dir="src/data/output"):
        """
        Initialize the environment deviation analyzer.
        
        Args:
            input_dir: Directory containing input files (output from extraction process)
            output_dir: Directory for output files
        """
        self.name = "EnvironmentDeviationAnalyzer"
    ...
    def find_latest_input_file(self):
        """
        Find the latest Environment_Data file in the input directory.
        
        Returns:
            bool: True if a file was found, False otherwise
        """
        try:
            # Look for files matching the pattern *Environment_Data.xlsx
            pattern = os.path.join(self.input_dir, "*Environment_Data.xlsx")
            matching_files = glob.glob(pattern)
            
            if not matching_files:
                self.logger.error(f"No Environment_Data files found in {self.input_dir}")
                return False
            ...
            self.client_name = filename.split('_Environment_Data')[0]
    ...
    def load_data(self):
        ...
        try:
            ...
            # Check if Environment_Register sheet exists
            if "Environment_Register" not in excel_file.sheet_names:
                self.logger.error("Required sheet 'Environment_Register' not found in input file")
                return False
            
            # Load the Environment_Register sheet
            self.env_data = pd.read_excel(excel_file, sheet_name="Environment_Register")
            
            # Check if required columns exist
            if "System Name" not in self.env_data.columns or "Environment Type" not in self.env_data.columns:
                self.logger.error("Required columns 'System Name' or 'Environment Type' not found in Environment_Register")
                return False
            
            self.logger.info(f"Loaded {len(self.env_data)} records from {self.input_file}")
            return True
    ...
    def analyze_environment_deviations(self):
        """
        Analyze environment deviations using Azure AI Foundry.
        
        Returns:
            bool: True if analysis was successful, False otherwise
        """
        try:
            if self.env_data is None or self.env_data.empty:
            ...
            self.logger.error(f"Error analyzing environment deviations: {str(e)}")
            self.logger.error(f"Traceback: {traceback.format_exc()}")
            return False   

    def _prepare_data_for_analysis(self):
        """
        Prepare data for environment deviation analysis.
        
        Returns:
            DataFrame: Prepared data for analysis
        """
        # Group by System Name to get unique systems
        systems = self.env_data['System Name'].unique()
        
        # Create a list of dictionaries with system name and its environments
        data_for_analysis = []
        for system in systems:
            system_envs = self.env_data[self.env_data['System Name'] == system]['Environment Type'].tolist()
            data_for_analysis.append({
                'System Name': system,
                'Environments': system_envs
            })
    ...
    def _process_batches_with_ai(self, data):
        ...
        for i in range(0, len(data), batch_size):
            ...
            prompt = self._create_environment_analysis_prompt(batch_json)
    ...
    async def _call_ai_for_analysis_async(self, prompt, batch_index, batch_size, total_records):
        ...
        try:
            ...
            system_prompt = """You are an expert IT environment segregation analyst specializing in compliance and environment validation. 
            Your task is to analyze system environments and identify deviations from the required DEV, TEST, PROD environment setup."""
    ...
    def _create_environment_analysis_prompt(self, data_json):
        """
        Create the prompt for environment analysis.
        
        Args:
            data_json: JSON string containing environment data
            
        Returns:
            String prompt for the AI model
        """
        return f"""
        You are an expert in IT environment segregation analysis. I need you to analyze the following system environment data:
        ...
        1. For each System Name, check which Environment Types (DEV, TEST, PROD) are present in the 'Environments' list
        2. If any environment is missing, mark it as "Deviation" with the reason "No [ENVIRONMENT] environment available"
           (list all missing environments if multiple are missing)
        3. If all three environments (DEV, TEST, PROD) are present, mark it as "OK" with reason "DEV, TEST, PROD environments are present"
        
        REQUIRED OUTPUT FORMAT:
        A JSON array containing one object for each System Name, with these exact fields:
        - System_Name: The system name
        - Environment_DTAP: Either "Deviation" or "OK"
        - Reason: The reason for deviation, or "DEV, TEST, PROD environments are present" if OK
        
        EXAMPLE OUTPUT:
        [
            {{"System_Name": "SAP FI", "Environment_DTAP": "OK", "Reason": "DEV, TEST, PROD environments are present"}},
            {{"System_Name": "Oracle EBS AP", "Environment_DTAP": "Deviation", "Reason": "No TEST environment available"}}
        ]
        
        CRITICAL: You MUST analyze EVERY System Name in the input data. Do not skip any systems.
    ...
    def _call_ai_with_thread_and_run(self, system_prompt, user_prompt, max_retries=3):
        ...
        while retry_count < max_retries:
            try:
                ...
                system_instructions = f"You are an expert in IT environment segregation analysis. {system_prompt}"
        ...
        try:
            ...
            if not agents_list:
                ...
                agent = self.client.create_agent(
                    name="Environment Deviation Analyzer",
                    description="Analyzes environment deviations in IT systems",
                    model=self.AGENT_MODEL_DEPLOYMENT_NAME
                )
                return agent.id
            else:
    ...
    def save_analysis_results(self):
        """
        Save the analysis results to a new Excel file with input data and analysis results.
        ...
        try:
            ...
            if not self.input_file or not self.env_data.any().any():
            ...
            with pd.ExcelWriter(output_file_path, engine='openpyxl') as writer:
                # Write the input environment data to the first sheet
                self.env_data.to_excel(writer, sheet_name="Environment_Register_Input", index=False)
                
                # Write the analysis results to the second sheet
                self.analysis_results.to_excel(writer, sheet_name="Environment_Deviation_Analysis", index=False)
    ...
    def _create_metadata(self):
        ...
        if self.analysis_results is not None and not self.analysis_results.empty:
            ...
            exception_records = len(self.analysis_results[self.analysis_results['Environment_DTAP'] == 'Deviation'])
            ok_records = len(self.analysis_results[self.analysis_results['Environment_DTAP'] == 'OK'])
    ...
    def run(self):
        """
        Run the complete environment deviation analysis workflow.
        ...
        try:
            ...
            # Step 2: Analyze environment deviations
            success = self.analyze_environment_deviations()
            if not success:
                self.logger.error("Failed to analyze environment deviations")
                return False
            ...
            self.logger.info(f"Environment Deviation Analysis completed successfully. Results saved to {output_file}")
            return True
